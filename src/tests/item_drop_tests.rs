...
use crate::{create_stat_list, game_engine::{entity_components::CollisionBox, game::MousePosition, item::{Item, ItemArchetype, ItemType}, loot::{LootTable, LootTableEntry}, stat::GearStatList}, tests::{lib::headless::HeadlessGame, test_framework::{basic_camera, basic_world}}};
#[tokio::test]
pub async fn test_enemy_item_drops() {
    ...
    if let Err(e) = headless.run(500).await {
         panic!("{}", e)
    }
    assert!(
